package online.ahassan.questionservice.services;
...
import online.ahassan.questionservice.repositories.QuestionOptionsRepository;
import online.ahassan.questionservice.repositories.QuestionRepository;
import online.ahassan.questionservice.repositories.QuizQuestionRepository;
...
public class QuestionService {
    ...
    public Page<QuestionDto> findByQuizId(Integer quizId, Pageable pageable) {
        Page<QuizQuestion> quizQuestionIds = quizQuestionRepository.findByQuizId(quizId, pageable);
        List<QuestionDto> retrievedQuestions = questionRepository.findAllById(
                quizQuestionIds.getContent().stream()
                        .map(QuizQuestion::getQuestionId)
                        .toList()
        ).stream().map(QuestionDto::fromEntity).toList();
        return new PageImpl<>(retrievedQuestions, quizQuestionIds.getPageable(), retrievedQuestions.size());
    }
    ...
    public QuizQuestionDto addQuestionToQuiz(QuizQuestionDto quizQuestionDto) {

        questionRepository.findById(quizQuestionDto.getQuestionId()).orElseThrow(() -> {
